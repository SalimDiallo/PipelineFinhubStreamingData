...
Streamlit + Plotly & Tableau de bord BI (restitution) \\
...
% ==================================================================
\section{Restitution — Dashboard BI}
% ==================================================================

\subsection{Objectif}

Pour valoriser les données modélisées, un \textbf{tableau de bord interactif} a
été développé avec \textbf{Streamlit}. Il offre une lecture immédiate des
métriques de marché issues de la couche analytique, sans requête SQL manuelle.

\subsection{Source de données}

Le dashboard interroge directement la base \textbf{DuckDB} produite par dbt
(\code{finhub.duckdb}), en \textbf{lecture seule}. Ce mode évite tout verrou en
écriture et n'interfère donc pas avec les exécutions \code{dbt run}. Les
requêtes sont mises en cache pour la réactivité.

\subsection{Contenu}

\begin{itemize}
  \item \textbf{Indicateurs clés} (KPIs) : nombre de symboles suivis, total des
        trades, volume total, volatilité moyenne ;
  \item \textbf{Trades par symbole} et \textbf{volatilité par symbole}
        (diagrammes à barres) ;
  \item \textbf{Évolution du prix} dans le temps (courbes, à partir de
        \code{fact\_trades}) ;
  \item \textbf{Tableau détaillé} des métriques \code{agg\_stock\_metrics} ;
  \item \textbf{Filtre} interactif par symbole.
\end{itemize}

Les visualisations sont réalisées avec Plotly et adoptent une palette en
niveaux de gris, cohérente avec l'identité visuelle du projet.

\subsection{Mise en route}

\begin{lstlisting}
make dashboard          # ou : uv run streamlit run dashboard/app.py
# -> http://localhost:8501
\end{lstlisting}

Prérequis : les modèles dbt doivent avoir été construits (\code{dbt run}) afin
que la base \code{finhub.duckdb} existe.

\subsection{Validation}

Le serveur Streamlit démarre correctement (réponse HTTP~200 sur le point de
santé) et les visualisations s'alimentent des données réelles de la couche
analytique (par exemple quatre symboles suivis et plusieurs centaines de trades
agrégés).
...
conteneurisation Docker. Un tableau de bord interactif (Streamlit) assure enfin
la restitution des métriques analytiques.
